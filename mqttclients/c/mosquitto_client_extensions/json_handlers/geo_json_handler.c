    ...
    geojson_point* output)
{
  ...
  RETURN_IF_NULL(type_str = (char*)json_object_get_string(type), jobj);
    ...
    mosquitto_payload* message)
{
  ...
  RETURN_IF_NULL(
      payload
      = json_object_to_json_string_length(jobj, JSON_C_TO_STRING_PLAIN, &message->payload_length), jobj);
