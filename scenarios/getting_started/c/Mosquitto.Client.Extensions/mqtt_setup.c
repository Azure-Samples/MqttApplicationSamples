...
void mqtt_client_set_connection_settings( mqtt_client_connection_settings * connection_settings )
{
    ...
    connection_settings->ca_path = getenv( "CA_PATH" ) ? : connection_settings->ca_file ? NULL : "/etc/ssl/certs";
    ...
    connection_settings->use_TLS = ( use_TLS != NULL && strcmp( use_TLS, "false" ) == 0 ) ? false : true; /* TODO: figure out "cat" case */
    ...
    connection_settings->clean_session = ( clean_session != NULL && strcmp( clean_session, "false" ) == 0 ) ? false : true; /* TODO: figure out "cat" case */
}
...
void on_mosquitto_log( struct mosquitto * mosq,
                     void * obj,
                     int level,
                     const char * str )
{
                             ...
                             mqtt_client_connection_settings * connection_settings )
{
    ...
    {
        result = mosquitto_username_pw_set( mosq, connection_settings->username, connection_settings->password );
    ...
    if( connection_settings->use_TLS )
    {
        ...
        result = mosquitto_tls_set( mosq, connection_settings->ca_file, connection_settings->ca_path, connection_settings->cert_file, connection_settings->key_file, NULL );
